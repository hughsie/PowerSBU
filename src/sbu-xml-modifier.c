			       ...
			       GError **error)
{
	...
	for (guint i = 0; attribute_names[i] != NULL; i++) {
		...
		if (text_replace != NULL) {
			g_string_append_printf(helper->out,
					       " %s=\"%s\"",
					       attribute_names[i],
					       text_replace);
			continue;
...
{
	GMarkupParser parser = {sbu_xml_modifier_start_element,
...
sbu_xml_modifier_replace_attr(SbuXmlModifier *self,
			      ...
			      const gchar *value)
{
	g_hash_table_insert(self->hash_cdata, g_strdup_printf("%s-%s", id, attr), g_strdup(value));
}
