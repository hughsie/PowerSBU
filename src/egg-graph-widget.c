...
egg_graph_widget_class_init(EggGraphWidgetClass *class)
{
	...
	g_object_class_install_property(
	    object_class,
	    PROP_USE_LEGEND,
	    g_param_spec_boolean("use-legend", NULL, NULL, FALSE, G_PARAM_READWRITE));
	g_object_class_install_property(
	    object_class,
	    PROP_USE_GRID,
	    g_param_spec_boolean("use-grid", NULL, NULL, TRUE, G_PARAM_READWRITE));
	g_object_class_install_property(object_class,
					PROP_TYPE_X,
					g_param_spec_uint("type-x",
							  NULL,
							  NULL,
	...
	g_object_class_install_property(
	    object_class,
	    PROP_AUTORANGE_X,
	    g_param_spec_boolean("autorange-x", NULL, NULL, TRUE, G_PARAM_READWRITE));
	g_object_class_install_property(
	    object_class,
	    PROP_AUTORANGE_Y,
	    g_param_spec_boolean("autorange-y", NULL, NULL, TRUE, G_PARAM_READWRITE));
	g_object_class_install_property(
	    object_class,
	    PROP_MIRROR_X,
	    g_param_spec_boolean("mirror-x", NULL, NULL, TRUE, G_PARAM_READWRITE));
	g_object_class_install_property(
	    object_class,
	    PROP_MIRROR_Y,
	    g_param_spec_boolean("mirror-y", NULL, NULL, TRUE, G_PARAM_READWRITE));
	g_object_class_install_property(object_class,
					PROP_START_X,
...
egg_graph_widget_init(EggGraphWidget *graph)
{
	...
	priv->legend_list =
	    g_ptr_array_new_with_free_func((GDestroyNotify)egg_graph_widget_key_legend_data_free);
...
egg_graph_widget_draw_labels(EggGraphWidget *graph, cairo_t *cr)
{
	...
	for (i = 0; i < 11; i++) {
		...
		cairo_move_to(cr, b - offsetx, priv->box_y + priv->box_height + 2.0);
...
egg_graph_widget_autorange_x(EggGraphWidget *graph)
{
	...
	g_debug("Processed(1) range is %.1f<x<%.1f", priv->start_x, priv->stop_x);
	...
	g_debug("Processed range is %.1f<x<%.1f", priv->start_x, priv->stop_x);
}
...
egg_graph_widget_autorange_y(EggGraphWidget *graph)
{
	...
	g_debug("Processed(1) range is %.1f<y<%.1f", priv->start_y, priv->stop_y);
	...
	g_debug("Processed range is %.1f<y<%.1f", priv->start_y, priv->stop_y);
}
	...
	for (j = 0; j < array->len; j++) {
		...
		if (plot == EGG_GRAPH_WIDGET_PLOT_LINE || plot == EGG_GRAPH_WIDGET_PLOT_BOTH) {
			...
			for (i = 1; i < data->len; i++) {
				...
				if (point->x < priv->start_x || point->x > priv->stop_x) {
					continue;
				}
 ...
 **/
static void
egg_graph_widget_draw_legend(EggGraphWidget *graph,
			     cairo_t *cr,
			     gint x,
			     gint y,
			     gint width,
 ...
 **/
static gboolean
egg_graph_widget_legend_calculate_size(EggGraphWidget *graph,
				       cairo_t *cr,
				       guint *width,
				       guint *height)
{
...
egg_graph_widget_draw(GtkWidget *widget, cairo_t *cr)
{
	...
	if (priv->use_legend && legend_height > 0) {
		priv->box_width = allocation.width - (3 + legend_width + 5 + priv->box_x);
		legend_x = priv->box_x + priv->box_width + 6;
		legend_y = priv->box_y;
	} else {
		priv->box_width = allocation.width - (3 + priv->box_x);
	}

	/* graph background */
	egg_graph_widget_draw_bounding_box(cr,
					   priv->box_x,
					   priv->box_y,
					   priv->box_width,
					   priv->box_height);
	...
	if (priv->use_legend && legend_height > 0)
		egg_graph_widget_draw_legend(graph,
					     cr,
					     legend_x,
					     legend_y,
					     legend_width,
...
egg_graph_widget_export_to_svg_cb(void *user_data, const unsigned char *data, unsigned int length)
{
